...
pub enum LoggerConfig {
    ...
    Scylla(String, String, String, String, String, u64)
}

pub fn new_logger(config: LoggerConfig) -> Result<Arc<dyn Logger>, CommonError> {
    match config {
        LoggerConfig::File(name,level, size) => {
            Ok(Arc::new(FileLogger::new(&name, level, size)?))
        },
        LoggerConfig::Scylla(name, addr, dbname, user, passwd, ttl) => {
            Ok(Arc::new(ScyllaLogger::new(name, addr, dbname, user, passwd, ttl)?))
        }
    }
}